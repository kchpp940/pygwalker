...
const ExportConfigModal: React.FC = observer(() => {
    ...
    useEffect(() => {
        if (commonStore.exportConfigModalOpen && commonStore.storeRef?.current) {
            const spec = commonStore.storeRef.current.exportCode();
            setVisSpec(spec);
        }
    }, [commonStore.exportConfigModalOpen]);
    ...
    const closeModal = useCallback(() => {
        commonStore.closeModal("exportConfig");
    }, []);

    const getCurrentVisSpec = () => {
        if (!commonStore.storeRef?.current) return [];
        const allSpec = commonStore.storeRef.current.exportCode();
        
        if (!exportMultipleCharts && commonStore.storeRef.current.visIndex !== undefined) {
            return [allSpec[commonStore.storeRef.current.visIndex]];
        }
        return allSpec;
    };
    ...
    const exportImages = async () => {
        if (!commonStore.gwRef?.current) return;
        
        const gwMode: "data-url" | "svg" = exportFormat === "svg" ? "svg" : "data-url";
        const imgFormat: "png" | "svg" = exportFormat === "svg" ? "svg" : "png";

        if (exportMultipleCharts && commonStore.gwRef.current.exportChartList) {
            const allCharts: IChartExportResult[] = [];
            for await (const chart of commonStore.gwRef.current.exportChartList(gwMode)) {
                allCharts.push(chart.data);
            }
            await exportMultipleImages(imgFormat, allCharts);
        } else {
            const chartData = await commonStore.gwRef.current.exportChart!(gwMode);
    ...
    const exportPythonCode = async () => {
        ...
        if (commonStore.sourceInvokeCode) {
            code += `\n# Original source code:\n# ${commonStore.sourceInvokeCode.split('\n').join('\n# ')}\n\n`;
        }
    ...
    return (
        <Dialog
            open={commonStore.exportConfigModalOpen}
            modal={false}
            onOpenChange={(show) => {
                show ? commonStore.openModal("exportConfig") : commonStore.closeModal("exportConfig");
            }}
        >
